...
func newTokenIssuer(jwtSecret []byte, accessTTL, refreshTTL time.Duration) (*tokenIssuer, error) {
	if len(jwtSecret) == 0 {
		return nil, fmt.Errorf("app: jwt secret is empty")
	}
	if accessTTL <= 0 {
...
func (t *tokenIssuer) mintAccess(userID, username string) (token string, err error) {
	now := time.Now().UTC()
	...
	if signErr != nil {
		return "", fmt.Errorf("app: sign access token: %w", signErr)
	}
	return signed, nil
}
...
func (t *tokenIssuer) mintRefresh(userID, username string) (token string, jti string, err error) {
	...
	signed, signErr := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString(t.secret)
	if signErr != nil {
		return "", "", fmt.Errorf("app: sign refresh token: %w", signErr)
	}
...
func (t *tokenIssuer) parseRefresh(token string) (userID, username, jti string, err error) {
	parsed, err := jwt.Parse(token, func(token *jwt.Token) (any, error) {
		if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("app: unexpected signing method: %v", token.Header["alg"])
		}
		return t.secret, nil
	})
	if err != nil || !parsed.Valid {
		return "", "", "", fmt.Errorf("app: parse refresh token: %w", err)
	}

	claims, ok := parsed.Claims.(jwt.MapClaims)
	if !ok {
		return "", "", "", fmt.Errorf("app: refresh claims type")
	}
	if typ, _ := claims["typ"].(string); typ != claimTypeRefresh {
		return "", "", "", fmt.Errorf("app: refresh token typ")
	}
	...
	if sub == "" || jtiClaim == "" {
		return "", "", "", fmt.Errorf("app: refresh token missing sub/jti")
	}
